...
// BEGIN: Val
void DomUI::readUpdated( QXmlStreamReader &reader )
{
    for ( bool finished = false; !finished && !reader.hasError(); )
    {
        switch ( reader.readNext() )
        {
            case QXmlStreamReader::StartElement :
            {
                ...
                if ( tag == QLatin1String( "triggers" ) )
                {
                    DomTriggers *v = new DomTriggers();
                    v->read( reader );
                    setElementTriggers( v );
                    continue;
                }
                ...
                reader.raiseError( QLatin1String( "readUpdated: Unexpected element " ) + tag );
            } break;
...
#ifdef QUILOADER_QDOM_READ
void DomUI::readUpdated( const QDomElement &node )
{
...
void DomUI::write(QXmlStreamWriter &writer, const QString &tagName) const
{
    ...
    // BEGIN: Val
    if ( m_children & Triggers )
    {
        m_triggers->write( writer, QLatin1String( "triggers" ) );
    }
    // END: Val
...
// BEGIN: Val
DomTriggers* DomUI::takeElementTriggers()
{
    DomTriggers* a = m_triggers;
    m_triggers = 0;
    m_children ^= Triggers;
    return a;
}

void DomUI::setElementTriggers( DomTriggers* a )
{
    delete m_triggers;
    m_children |= Triggers;
    m_triggers = a;
...
// BEGIN: Val
void DomUI::clearElementTriggers()
{
    delete m_triggers;
    m_triggers = 0;
    m_children &= ~Triggers;
...
// BEGIN: Val
void DomTriggers::clear( bool clear_all )
{
    qDeleteAll( m_trigger );
    m_trigger.clear();
...
void DomTriggers::read( QXmlStreamReader &reader )
{
    for ( bool finished = false; !finished && !reader.hasError(); )
    {
        switch ( reader.readNext() )
        {
            ...
            {
                const QString tag = reader.name().toString().toLower();
                if ( tag == QLatin1String( "trigger" ) )
                {
                    DomTrigger *v = new DomTrigger();
                    v->read( reader );
                    m_trigger.append( v );
                    continue;
                }
...
#ifdef QUILOADER_QDOM_READ
void DomTriggers::read( const QDomElement &node )
{
    for ( QDomNode n = node.firstChild(); !n.isNull(); n = n.nextSibling() )
    {
        if ( !n.isElement() )
...
void DomTriggers::write( QXmlStreamWriter &writer, const QString &tagName ) const
{
    writer.writeStartElement( tagName.isEmpty() ? QString::fromUtf8( "triggers" ) : tagName.toLower() );

    for ( int i = 0; i < m_trigger.size(); ++i )
    {
        DomTrigger* v = m_trigger[i];
        v->write( writer, QLatin1String( "trigger" ) );
    }
...
void DomTriggers::setElementTrigger( const QList< DomTrigger* >& a )
{
    m_children |= Trigger;
    m_trigger = a;
}

void DomTrigger::clear( bool clear_all )
{
    qDeleteAll( m_event );
    m_event.clear();
...
void DomTrigger::read( QXmlStreamReader &reader )
{
    foreach ( const QXmlStreamAttribute &attribute, reader.attributes() )
    {
        QStringRef name = attribute.name();
        ...
        {
            setAttributeObject( attribute.value().toString() );
            continue;
        }
...
#ifdef QUILOADER_QDOM_READ
void DomTrigger::read( const QDomElement &node )
{
    if ( node.hasAttribute( QLatin1String( "object" ) ) )
        setAttributeObject( node.attribute( QLatin1String( "object" ) ) );
...
void DomTrigger::write( QXmlStreamWriter &writer, const QString &tagName ) const
{
    writer.writeStartElement( tagName.isEmpty() ? QString::fromUtf8( "trigger" ) : tagName.toLower() );

    if ( hasAttributeObject() )
        writer.writeAttribute( QLatin1String( "object" ), attributeObject() );

    for ( int i = 0; i < m_event.size(); ++i )
    {
        DomEvent *v = m_event[i];
        v->write( writer, QLatin1String( "event" ) );
    }
...
void DomTrigger::setElementEvent( const QList< DomEvent* > &a )
{
    m_children |= Event;
    m_event = a;
}

void DomEvent::clear( bool clear_all )
{
    if ( clear_all )
    {
        m_text = QLatin1String( "" );
        m_has_attr_signal = false;
        m_has_attr_action = false;
    }
...
void DomEvent::read( QXmlStreamReader &reader )
{
    foreach ( const QXmlStreamAttribute &attribute, reader.attributes() )
    {
        QStringRef name = attribute.name();
        ...
        {
            setAttributeSignal( attribute.value().toString() );
            continue;
        }
        else if ( name == QLatin1String( "action" ) )
        {
            setAttributeAction( attribute.value().toString() );
            continue;
        }
...
#ifdef QUILOADER_QDOM_READ
void DomEvent::read( const QDomElement &node )
{
    if (node.hasAttribute( QLatin1String( "signal" ) ) )
        setAttributeSignal( node.attribute( QLatin1String( "signal" ) ) );

    if (node.hasAttribute( QLatin1String( "action" ) ) )
        setAttributeAction( node.attribute( QLatin1String( "action" ) ) );
...
void DomEvent::write( QXmlStreamWriter &writer, const QString &tagName ) const
{
    writer.writeStartElement( tagName.isEmpty() ? QString::fromUtf8( "event" ) : tagName.toLower() );

    if ( hasAttributeSignal() )
        writer.writeAttribute( QLatin1String( "signal" ), attributeSignal() );

    if ( hasAttributeAction() )
        writer.writeAttribute( QLatin1String( "action" ), attributeAction() );
